...
ASSET_MODEL_PATH  = os.getenv("ASSET_MODEL_PATH",  "app/models/asset-detection-best.pt")
DEFECT_MODEL_PATH = os.getenv("DEFECT_MODEL_PATH", "app/models/defect-detection-best.pt")
...
WITH target_inspection AS (
    SELECT insp_sub.id AS inspection_id
    FROM pvx_file f_sub
    JOIN pvx_inspection_file inf_sub ON inf_sub.file_id = f_sub.id
    JOIN pvx_inspection insp_sub ON insp_sub.id = inf_sub.inspection_id
...
JOIN pvx_inspection insp ON insp.id = inf.inspection_id
WHERE f.status = 'uploaded'
  AND f.file_type = 'groundbasemobileimages'
  AND f.is_deleted = false
  AND insp.id = (SELECT inspection_id FROM target_inspection)
...
def _output_key(s3_url: str, tower_id: str, inspection_id: str, task_type: str) -> str:
    folder   = os.getenv("FOLDER_NAME", "PIEFLY_QAT").strip("'\"")
